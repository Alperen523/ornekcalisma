...
using UzmanCrm.CrmService.WebAPI;
using UzmanCrm.CrmService.WebAPI.CustomAttributes.Swagger;
...
namespace UzmanCrm.CrmService.WebAPI
{
    ...
    public class SwaggerConfig
    {
                ...
                .EnableSwagger(c =>
                    {
                        ...
                        //
                        c.SingleApiVersion("v1", "UzmanCrm.CrmService.WebAPI");
                        ...
                        c.IncludeXmlComments(string.Format(@"{0}\bin\UzmanCrm.CrmService.WebAPI.xml", AppDomain.CurrentDomain.BaseDirectory));
                ...
                .EnableSwaggerUi(c =>
                    {
                        ...
                        //
                        c.DocumentTitle("KD API Swagger UI");
        ...
        {
            return System.AppDomain.CurrentDomain.BaseDirectory + @"bin\UzmanCrm.CrmService.WebAPI.xml";
        }
        ...
        public class ApplyDocumentFilter : IDocumentFilter
        {
            public void Apply(SwaggerDocument swaggerDoc, SchemaRegistry schemaRegistry, IApiExplorer apiExplorer)
            {
                ...
                var excludeRoutes = swaggerDoc.paths.Where(_ => _.Key == "/api/contact/save-customer").ToList();
                excludeRoutes.ForEach(_ => { swaggerDoc.paths.Remove(_.Key); });
                //var excludeRoutes1 = swaggerDoc.paths.Where(_ => _.Key == "/api/contact/save-customer-ecom").ToList();
                //excludeRoutes1.ForEach(_ => { swaggerDoc.paths.Remove(_.Key); });
                //var excludeRoutes2 = swaggerDoc.paths.Where(_ => _.Key == "/api/contact/get-customer-by-ecomid").ToList();
                //excludeRoutes2.ForEach(_ => { swaggerDoc.paths.Remove(_.Key); });
            ...
            private Info GetInfo()
            {
                ...
                var Url = isTest ? "https://crmapi-test.kahvedunyasi.com/doc/api/" : "https://crmapi.kahvedunyasi.com/doc/api/";


                return new Info
                {
                    title = "KD (CrmService Document) - API",
                    version = SwaggerApiVersion.Version,
                    // description = GetInfoDescription(),
                    termsOfService = "https://www.uzmancrm.com/",
                    contact = new Contact()
                    {
                        email = "ozge.ozbek@uzmancrm.com",
                        name = "Support",
                        url = Url
                    },
                    vendorExtensions = new Dictionary<string, object> {
            { "logo", new { url = "./icon.png", } }
          }
                };
            }
                ...
                var descriptions = new List<object>
        {
          ...
          new
          {
            ...
            private string GetInfoDescription()
            {
                ...
                string AddRedocfileContents = AppDomain.CurrentDomain.GetData("DataDirectory").ToString();
                //var AddRedocfileContents = Path.Combine(Directory.GetCurrentDirectory(), @"App_Data");
                AddRedocfileContents = Path.Combine(AddRedocfileContents, "RedocRenderMenu.html");
                AddRedocfileContents = System.IO.File.ReadAllText(AddRedocfileContents);
                var description = AddRedocfileContents;
